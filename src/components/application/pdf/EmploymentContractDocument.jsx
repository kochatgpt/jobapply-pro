    ...
    return (
        ...
        >
            ...
            <div className="mb-4 leading-[1.4]">
                ...
                <p className="text-justify">1.6 สังกัดหน่วยงาน<span className={`border-b border-dotted border-slate-400 inline-block min-w-[150px] text-center px-2 ${formData.department ? 'pb-1' : ''}`} style={{ verticalAlign: 'baseline', ...(!formData.department && { minHeight: '1.2em' }) }}>{formData.department || ''}</span>แผนก/ฝ่าย<span className={`border-b border-dotted border-slate-400 inline-block min-w-[150px] text-center px-2 ${formData.division ? 'pb-1' : ''}`} style={{ verticalAlign: 'baseline', ...(!formData.division && { minHeight: '1.2em' }) }}>{formData.division || ''}</span></p>
