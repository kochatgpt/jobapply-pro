        ...
        >
            ...
            <div className="mb-1 text-justify leading-[1.4]">
                ...
                <p>
                    ...
                    FAX. 662-860-6617 ซึ่งต่อไปนี้ในสัญญานี้เรียกว่า "บริษัท"ให้เริ่มปฏิบัติงานในวันที่<span className="border-b border-dotted border-slate-400 inline-block min-w-[100px] text-start px-2" style={{ verticalAlign: 'baseline', minHeight: '1.2em' }}>{formData.startDate ? new Date(formData.startDate).toLocaleDateString('th-TH') : '\u00A0'}</span> ตำแหน่ง
                    <span className="border-b border-dotted border-slate-400 inline-block min-w-[170px] text-start px-2" style={{ verticalAlign: 'baseline', minHeight: '1.2em' }}>{formData.position || '\u00A0'}</span> แผนก
                    <span className="border-b border-dotted border-slate-400 inline-block min-w-[170px] text-start px-2" style={{ verticalAlign: 'baseline', minHeight: '1.2em' }}>{formData.department || '\u00A0'}</span> ตกลงและ
                </p>
            </div>
            ...
            <div className="mb-4 text-justify leading-[1.4] space-y-2">
                <p className="indent-8">
                    ยินยอมเข้ารับการฝึกอบรม "หลักสูตรปฐมนิเทศพนักงานใหม่" ตามระเบียบของบริษัทฯมีกำหนดระยะเวลาการฝึกอบรม 3 วัน(ไม่นับรวมวันหยุด) เริ่มตั้งแต่วันที่<span className="border-b border-dotted border-slate-400 inline-block min-w-[100px] text-end px-2" style={{ verticalAlign: 'baseline', minHeight: '1.2em' }}>{formData.trainingStartDate ? new Date(formData.trainingStartDate).toLocaleDateString('th-TH') : '\u00A0'}</span>ถึงวันที่<span className="border-b border-dotted border-slate-400 inline-block min-w-[100px] text-end px-2" style={{ verticalAlign: 'baseline', minHeight: '1.2em' }}>{formData.trainingEndDate ? new Date(formData.trainingEndDate).toLocaleDateString('th-TH') : '\u00A0'}</span>โดยไม่ได้รับค่าจ้างหรือค่าตอบแทนใดๆ เพื่อเป็นการเตรียมความพร้อมให้กับพนักงานใหม่ก่อนเริ่มปฏิบัติงานจริงโดยการฝึกอบรมแบ่ง
                </p>
